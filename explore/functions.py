...
def fetch_card_image_arr(property_list: List[Property], get_all:bool=False)-> List[str]:
    ...
    for i in range(len(property_list)):
        if (i <= 1 or i == len(property_list) - 1) or get_all:
            ...
            if saved_qs.exists():
                image = saved_qs[0].image if saved_qs[0].image else google_street_view_api_base64(address=property_obj['address'])
            else:
                image = google_street_view_api_base64(address=property_obj['address'])
